...
var (
	...
	PublicKeyPrefixCompat = "ACC"

	MainnetPublic = addressEncoder.AddressType{"acc", addressEncoder.BTCAlphabet, "ripemd160", "", 33, []byte(PublicKeyPrefixCompat), nil}
...
func (dec *AddressDecoderV2) AddressDecode(pubKey string) ([]byte, error) {
	...
	} else if strings.HasPrefix(pubKey, PublicKeyPrefixCompat) { // "ACC"
		pubKeyMaterial = pubKey[len(PublicKeyPrefixCompat):] // strip "ACC"
	} else {
